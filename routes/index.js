...
router.get('/instructions', ensureAuthenticated, (req, res) => {
  ...
  router.post('/form1', ensureAuthenticated, (req, res) => {
    ...
    var body = _.pick(req.body, ['advNo', 'transactionId', 'bankName', 'branchName', 'date', 'ifsc', 'amount', "name", "motherName", "fatherName", "houseNoResidential", "streetResidential", "cityResidential", "landmarkResidential", "districtResidential", "postResidential", "policeResidential", "stateResidential", "countryResidential", "pincodeResidential", "houseNoPermanent", "streetPermanent", "cityPermanent", "districtPermanent", "postPermanent", "policePermanent","statePermanent", "countryPermanent", "landmarkPermanent", "pincodePermanent", "email", "mobileNo", "gender", "DOB", "age", "maritalStatus", "category", "pwd", "disability"])
    body.userId = userId
    clickedForm1 = true
...
router.get('/formFinal',ensureAuthenticated,  (req, res) => {
  ...
  Form1.findOne({userId}, (err, doc) => {
    body = _.pick(doc, ['advNo', 'transactionId', 'bankName', 'branchName', 'date', 'ifsc', 'amount', "name", "motherName", "fatherName", "houseNoResidential", "streetResidential", "cityResidential", "districtResidential", "stateResidential", "countryResidential", "pincodeResidential", "houseNoPermanent", "streetPermanent", "cityPermanent", "districtPermanent", "statePermanent", "countryPermanent", "pincodePermanent", "email", "mobileNo", "gender", "DOB", "age", "maritalStatus", "category", "pwd", "disability"])
...
router.get('/admins/userdetail/:id', ensureAuthenticatedAdmin, (req, res) => {
  ...
  router.post('/admins/establishmentAdmin/post', (req, res) => {
    var post = _.pick(req.body, ["post"])
    var postApplied = new Post(
     post
    )
    postApplied.save((err) => {
      if(err) return console.log(err)
      res.redirect('/admins/establishmentAdmin')
    })
  })

  router.get('/admins/establishmentAdmin/post/:id', (req, res) => {
    var id = req.params.id
    Post.findOneAndRemove({_id: id}, (err, doc) => {
      if(err) return console.log(err)
      console.log(doc)
      res.redirect('/admins/establishmentAdmin')
    } )
