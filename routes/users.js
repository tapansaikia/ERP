...
router.post('/register', (req, res) => {
  ...
  for(var i=0;i<password.length; i++) {
    for(var j=0; j<spcl.length; j++) {
      if(password[i]===spcl[j]){
        flag = true
        break;
      }
      if(flag===true)
  ...
  if (!flag || password.length < 8) {
    errors.push({ msg: 'Password must be at least 8 characters and must contain some special characters' });
  }  
    ...
    User.findOne({ email: email }).then(user => {
      if (user) {
        ...
        transporter.sendMail(HelperOptions, (error, info) => {
          if (error) {
            console.log(error)
            User.findOneAndRemove({email : newUser.email}, (err, doc) => {
